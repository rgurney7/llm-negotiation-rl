...
The honest framing: single-turn GRPO transfer to multi-turn is real, but it's same-distribution transfer. The model learns features that help within the distribution it trained on. SFT, trained on the same synthetic distribution, did improve on Craigslist, which is out-of-distribution for it, so cross-distribution transfer is possible from this data. I do not draw a comparative conclusion about why GRPO did not, because the GRPO Craigslist result's provenance is not verifiable from the persisted artifacts.
...
Before reading too much into the comparison, SFT had two structural advantages worth naming. First, curation: it trained only on positive-reward synthetic trajectories, so every gradient step pulled toward a known-good outcome. The RL agents had to discover what good looks like from scratch, with most rollouts producing zero or negative reward. Second, signal density: SFT got a per-token loss on every seller turn, while PPO had to propagate a single terminal scalar through 8 turns and GRPO had to find signal inside groups of mostly-failing completions. What SFT did not have was a data distribution advantage. It trained on synthetic scenarios and still improved on Craigslist, which is out-of-distribution for it. I do not extend that into a comparison against the RL agents, since their Craigslist eval provenance is unverifiable.
